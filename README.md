...
The decoder uses 16 interleaved streams. 24-bit integers are used to enable very fast lossless (non-approximated) vectorized divides using `_mm_div_ps`. No special signaling or sideband information is needed between the encoder and decoder. The encoder swizzles each individual range encoder's output bytes into the proper order right after compression. SSE 4.1 decoding is very fast on the Intel CPU's I've tried, at around 550-700 MiB/sec.
